import Node from "../../node/index";

describe("filter", () => {
    let c1: Node, c2: Node, c3: Node, c4: Node;
    beforeAll(() => {
        c1 = Node.createColumn(1);
        c2 = Node.createColumn(2);
        c3 = Node.createColumn(3);
        c4 = Node.createColumn(4);
        c1.connect("right", c2);
        c2.connect("right", c3);
        c3.connect("right", c4);

        expect(c1.right).toBe(c2);
        expect(c2.right).toBe(c3);
        expect(c3.right).toBe(c4);
        expect(c4.right).toBe(c1);
    });

    it("filters the nodes in the given direction matching the predicate", () => {
        expect(c1.filter("right", (n) => n.columnId < 4)).toEqual(
            expect.arrayContaining([c2, c3])
        );
        expect(c1.filter("left", (n) => n.columnId < 4)).toEqual(
            expect.arrayContaining([c2, c3])
        );
    });

    it("returns an empty array when no nodes match", () => {
        expect(c1.filter("right", (n) => n.columnId === 1)).toEqual([]);
    });
});
